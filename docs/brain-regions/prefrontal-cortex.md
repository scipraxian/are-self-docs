...
Are-Self's Prefrontal Cortex (`prefrontal_cortex/models.py`) does the same thing, except instead of treehouses, it organizes work for an AI team. When the system has something to do, the Prefrontal Cortex breaks it into a clean hierarchy: a big `PFCEpic` (like "build a new feature"), which splits into `PFCStory` records (smaller features), which split into `PFCTask` records (the actual work someone does right now). Each task has a `priority` (1=Critical through 4=Low), an `owning_disc` (the [IdentityDisc](./identity) assigned to it), and status tracking through `PFCItemStatus` — from `NEEDS_REFINEMENT` through `DONE`. Tasks can even link back to memories via `source_engrams`, connecting planning to what the system has learned.
