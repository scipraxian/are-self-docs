...
- **[Neuroplasticity](./neuroplasticity)**: Format-specific parser strategies are candidates to ship inside a [NeuralModifier](./neuroplasticity). The Unreal Engine log strategies are the canonical example — they'll ride along in the Unreal modifier at `neuroplasticity/genomes/unreal.zip` and register themselves with `LogParserFactory` at import time.
