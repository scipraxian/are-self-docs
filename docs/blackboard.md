...
- **Don't put giant objects on the board.** It gets deepcopied on every successor spike. A 10-megabyte JSON blob copied through a 50-spike train is a 500-megabyte nightmare. For anything large, store it in the [Hippocampus](./hippocampus) or write it to disk and put the path on the board.
...
- **[Central Nervous System](./central-nervous-system)**: the blackboard is a field on `Spike`, and the CNS is what deepcopies it forward, hands it to effectors, parses write-back lines, and serves it through the spike serializers to the UI.
- **[Frontal Lobe](./frontal-lobe)**: reasoning sessions read the blackboard via `resolve_environment_context` to assemble the `prompt` and other runtime variables, and write to it via the `mcp_update_blackboard` Parietal tool during the reasoning loop.
- **[Parietal Lobe](./parietal-lobe)**: hosts the `mcp_update_blackboard` MCP tool so the reasoning LLM can scribble on its own spike.
- **[Synaptic Cleft](./synaptic-cleft)**: every blackboard write fires an `Acetylcholine` neurotransmitter with `activity='blackboard_updated'` and a `vesicle` containing the key and value, so the UI can update in real time.
- **[Hippocampus](./hippocampus)**: for anything bigger than a note or longer-lived than a single spike train, the blackboard hands off to the engram store.
